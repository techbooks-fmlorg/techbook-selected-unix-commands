...
#@# X-TODO table 2.1 re-verify it
//table[table-chmod-number][絶対値指定]{
値	説明
----------------------------------------
400	所有者の読み込みを許可。
200	所有者の書き込みを許可。
100	ファイルの場合、所有者による実行を許可。 ディレクトリの場合、所有者による検索を許可。
040	グループのメンバからの読み込みを許可。
020	グループのメンバからの書き込みを許可。
010	ファイルの場合、グループのメンバによる実行を許可。
　	ディレクトリの場合、グループのメンバによる検索を許可。
004	他者からの読み込みを許可。
002	他者からの書き込みを許可。
001	ファイルの場合、他者による実行を許可。 ディレクトリの場合、他者による検索を許可。
//}

//table[table-chmod-symbol-who][シンボル指定(who)]{
だれ	説明
------------------------------------
u	ファイルの所有者許可ビット（に対しての操作）
g	ファイルのグループ許可ビット（に対しての操作）
o	ファイルの所有者とグループ以外の許可ビット（に対しての操作）
//}
