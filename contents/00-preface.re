 ...
 * @<term>{プロンプト((ぷろんぷと))} ... シェルが「入力待ち」状態を示す行左端の特殊文字。
                  例: @<B>{$}, @<B>{>}
 * @<term>{コマンド((こまんど))}   ... 対話処理の際にユーザが打ちこむ命令（プログラム名）
 ...
 * @<term>{BSD Unix}   ... (1) ARPAと契約してバークレイが開発していたAT&T Unixの改良版(〜4.3BSD Reno) (2) CSRG後期に、フリーソフトウエアとして作られた4.4BSDおよび4.4BSDの子孫たち。たいていは(2)の話をしている
 ...
 * @<term>{Public IP}  ... 世界で一意に割り当てられたIPアドレス。サイバースペースにおける住所に相当する。別名グローバルIP
