...
宛先にパケットを送信し、宛先までに通過する各ルータのIPアドレスを記録する。記録されたIPアドレスによって、宛先のホストまでの@<idx>{ネットワーク((ねっとわーく))<<>>経路を可視化((けいろをかしか))}するために利用する。
...
ネットワーク系のコマンドでは、
@<code>{@<idx>{-n}}オプションをつけて実行することを心がけるべきである
...
DNS障害に引きづられないよう、
DNSを使わずに@<idx>{ネットワーク((ねっとわーく))<<>>の動作確認((のどうさかくにん))}を行うべきである
