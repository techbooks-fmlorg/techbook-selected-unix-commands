...
後述するオプションのうち、整数を引数として指定するオプションにおいて、nを整数とすると、+nとするとnを超える数であることを意味し、-nとするとn未満であることを意味する。nとするとぴったりnであることを意味する。
...
ファイルの容量が指定した値であれば出力する。次の接尾辞を使用できる。
 ...
 @<strong>{b} ブロック。1ブロックは512バイトである。接尾辞を使用しない場合のデフォルトである。


 @<strong>{c} バイト。


 @<strong>{w} ワード。1ワードは2バイト。


 @<strong>{k} キロバイト。1キロバイトは1024バイト。


 @<strong>{M} メガバイト。1メガバイトは1048576バイト。


 @<strong>{G} ギガバイト。1ギガバイトは1073741824バイト。
...
探索したファイルを消去する。実行結果では、実行前と実行後にls -laを実行した出力を記述する。
...
探索したファイルに対して、指定したコマンドを実行する。-exec コマンド {} \;のように記述する。{}は探索した全てのファイル名に置き換えられる。コマンドの終わりには\;が必要。実行結果では、実行前と実行後にls -laを実行した出力を記述する。
