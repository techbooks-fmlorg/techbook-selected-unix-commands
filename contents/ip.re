...
#@# X-TODO CIDR verify "network prefix (bits)", ok?
最重要の確認項目は次の２つ。

 *  inet:	ipv4アドレス/プレフィックス（のビット長）が記載されている。
...
//note[column-ip][ipコマンドの読み方]{

ipコマンドをオブジェクト指向言語っぽい読み方をすれば、次のようになるだろう。
Unixはスペース区切りなので、これを上のようにスペース区切りで表記すると考えてほしい
//list[][][fontsize=xx-small]{
ip->OBJECT->COMMAND();
